...
for i in range(0,len(X.columns)):
    forward_list=list(X.columns)
    forward_list.reverse()
    X_train_tmp=X_train_tmp[forward_list]
    X_test_tmp=X_test_tmp[forward_list]
...
all_result.to_excel(path+'\\all_result_rcvonce_backward_temp.xlsx')
with open(path+'\\all_result_rcvonce_backward_temp.pickle', 'wb') as f:
